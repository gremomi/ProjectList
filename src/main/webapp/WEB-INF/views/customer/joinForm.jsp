...
$(function() {
	...
	$("input:radio[name=new_join]").click(function(){
		var check = $("input[name=new_join]:checked").val();
		if(check == 'admin') {
			var number = prompt("관리자 번호를 입력하세요.");
...
<%-- 	<c:url var="root" value="/" />
	...
	<a href="${root}">홈으로</a> --%>
		...
		<form>
			<fieldset>
	 			<legend><h2>회원 가입</h2></legend>
				<label for="general">일반 
				<input id="general" type="radio" name="new_join" value="general" checked="checked">
				</label>
				
				<label for="admin">관리자
				<input id="admin" type="radio" name="new_join" value="admin">
				</label>
			</fieldset>
		</form>
