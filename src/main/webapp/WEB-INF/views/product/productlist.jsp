...
$(function() {
	...
	$('.span2').on('slide', function (ev) {
        ...
        $(".betweenbtn").click(function(){
    		location.href="show?searchItem="+searchItem+"&searchWord="+searchWord+"&min="+min+"&max="+max;
        })
    });
	$("#doubleSearchbtn").click(function(){
		  var doubleSearch= $("#doubleSearch").val();
		  
		  location.href="show?searchItem="+searchItem+"&searchWord="+searchWord+"&min="+min+"&max="+max+"&doubleSearch="+doubleSearch;
			  })
});
</script>
<!-- /price range -->
...
function addtocart(pno, pimg, price, color, productname) {
  ...
  function input(resp) {
	  var data = ""
		...
		$.each(resp,function(index, item){
			...
			data +="<a href='#' class='btn btn-default add-to-cart'><i class='fa fa-shopping-cart'></i>Add to cart</a>"
			data +="</div>"
			data +="</div>"
			data +="</div>"
			data +="</div>"
				...
				<div class="col-sm-3">
					...
					<div class="col-sm-9 padding-right">
						<c:forEach var="product" items="${productList }">
								...
								<div class="product-image-wrapper">
									<div class="single-products">
										<div class="productinfo text-center">
											...
											<a href="productDetail?productno=${product.productno }" class="btn btn-default add-to-cart"><i
												class="fa fa-shopping-cart"></i>상세보기</a>
										</div>
									</div>
									<div class="choose">
										<ul class="nav nav-pills nav-justified">
											<li><a href="javascript:addtocart(${product.productno},'${product.productimg}',${product.price},'${product.color}','${product.productname}')"><i class="fa fa-plus-square"></i>Add
													to cart</a></li>
										</ul>
									</div>
								</div>
							...
							<input id="doubleSearch" type="text" class="form-control" style="position: relative; width: 200px; display: inline; margin-right: 1px;"><button id="doubleSearchbtn" class="btn btn-primary" style="margin-bottom: 16px;">검색</button>	<p></p>
	<ul class="pagination justify-content-center" style="margin:20px 0" >
    <li class="page-item"><a class="page-link" href="show?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.currentPage-navi.pagePerGroup}">⇤</a></li>
    <li class="page-item"><a class="page-link" href="show?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.currentPage-1}">←</a></li>
    <c:forEach var="currentPage" begin="${navi.startPageGroup }" end="${navi.endPageGroup }">
    
    <li class="page-item ${currentPage==navi.currentPage?'active':''}" ><a class="page-link" href="show?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${currentPage}">${currentPage}</a></li>
