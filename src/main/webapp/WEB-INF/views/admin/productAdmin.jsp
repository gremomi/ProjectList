				...
				<div class="col-sm-9">
					<div class="product-details"><!--product-details-->
								...
								<table class="table table-condensed">
									...
									<c:forEach var="list" items="${list}" varStatus="stat">
										<tr>
											...
											<td><img class="product_img" alt="${list.productname}" src="${list.productimg}"/></td>
											<td>
												<a href="productDetail?productno=${list.productno}&searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.currentPage}">${list.productname}</a>
											</td>
											<td>${list.cate}</td>
											<td>${list.price}</td>
										</tr>
                      ...
                      <div class="pagination-area">
                        <ul class="pagination">
                            <li class="page-item"><a href="productAll?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.countPerPage-navi.pagePerGroup}"><i class="fa fa-angle-double-left"></i></a></li> 
                            <!-- paging 위해  -->
                            <c:forEach var="currentPage" begin="${navi.startPageGroup}" end="${navi.endPageGroup}">
                               <c:if test="${navi.currentPage == currentPage}">
                                  <li><a href="productAll?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${currentPage}">${currentPage}</a></li>
                               </c:if>
                               <c:if test="${navi.currentPage != currentPage}">
                                  <li><a href="productAll?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${currentPage}">${currentPage}</a></li>
                               </c:if>
                            ...
                            <!-- 한 그룹 뒤로 가는 것 --> 
                           <li><a href="productAll?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.currentPage+navi.pagePerGroup}"><i class="fa fa-angle-double-right"></i></a></li>
                        </ul>
                     </div>
	 						<!-- 게시글 검색 -->
							<form action="productAll" method="GET">
								<select name="searchItem">
									<option value="productname" ${searchItem=='productname'?'selected':''}>상품명</option>
									<option value="cate" ${searchItem=='cate'?'selected':''}>카테고리</option>
									<option value="color" ${searchItem=='color'?'selected':''}>색</option>
								</select>
