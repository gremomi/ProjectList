	...
	<section>
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9">
					<div class="blog-post-area"><!--product-details-->
						<div class="table-responsive cart_info">
							...
							<c:if test="${not empty list}">
								<table class="table table-condensed">
									...
									<c:forEach var="list" items="${list}" varStatus="stat">
										...
										<tr class="inputdata">
											...
											<td><a href="deleteCustomer?custid=${list.custid}"><input id="deleteCustomer" type="button" value="삭제"></a></td>
										</tr>
									</c:forEach>
								</table>
							</c:if>
	 						...
	 						<div class="pagination-area">
								<ul class="pagination">
									<%-- <li class="page-item"><a class="page-link" href="Allcust?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.currentPage-navi.pagePerGroup}">previous</a></li> --%>
	 								<!-- paging 위해  -->
	 								<c:forEach var="currentPage" begin="${navi.startPageGroup}" end="${navi.endPageGroup}">
	 									<c:if test="${navi.currentPage == currentPage}">
	 										<li><a href="Allcust?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${currentPage}">${currentPage}</a></li>
	 									</c:if>
	 									<c:if test="${navi.currentPage != currentPage}">
	 										<li><a href="Allcust?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${currentPage}">${currentPage}</a></li>
	 									</c:if>
	 								...
	 								<!-- 한 그룹 뒤로 가는 것 --> 
									<li><a href="Allcust?searchItem=${searchItem}&searchWord=${searchWord}&currentPage=${navi.currentPage+navi.pagePerGroup}"><i class="fa fa-angle-double-right"></i></a></li>
								</ul>
							</div>
	 						<!-- 게시글 검색 -->
							<form id="search" action="Allcust" method="GET">
								<select name="searchItem">
									<option value="custid" ${searchItem=='custid'?'selected':''}>id</option>
									<option value="custname" ${searchItem=='custname'?'selected':''}>이름</option>
									<option value="custemail" ${searchItem=='custemail'?'selected':''}>email</option>
									<option value="custphone" ${searchItem=='custphone'?'selected':''}>전화번호</option>
								</select>
								<div class="search_box" style="margin-top:10px;margin-right:10px;display:inline-block;">
									<input type="text" name="searchWord" value="${searchWord}" />
								</div><input class="btn btn-success" type="submit" value="검색" />
